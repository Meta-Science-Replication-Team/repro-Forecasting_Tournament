...
computeMASE <- function(forecast, train, test, period) {
	...
	test <- as.vector(test)
	n <- length(train)
	scalingFactor <- sum(abs(train[(period + 1):n] - train[1:(n - period)])) / 
		(n - period)
...
wave1 <- read.csv(here::here("Data Cleaning",
							 "Wave1Forecasts-Cleaned_2021-05-17.csv"),
...
wave2 <- read.csv(here::here("Data Cleaning",
							 "Wave2Forecasts-Cleaned_2021-05-17.csv"), 
				  stringsAsFactors = FALSE)
...
demographics <- read.csv(here::here("Data Cleaning",
									"Team_Demographics.csv"), stringsAsFactors = FALSE)
...
dat_lay <- read.csv(here::here("Data Cleaning","Lay_Sample_Filtered_2021-05-18.csv"), stringsAsFactors = FALSE)
...
dat_comp <- read.csv(here::here("Data Cleaning","team_composition_2021-04-22.csv"), stringsAsFactors = FALSE)
...
dat_size <- read.csv(here::here("Data Cleaning","team_descriptives_updated.csv"), stringsAsFactors = FALSE)
...
dat_coded <- read.csv(here::here("Data Cleaning","Method+ComplexityCoding_fixed.csv"))
...
dat_counter <- read.csv(here::here("Data Cleaning","Counterfactuals_fixed.csv"), stringsAsFactors = FALSE)
...
dat_justification <- read.csv(here::here("Data Cleaning","review_justificationV2.csv"), stringsAsFactors = FALSE)
...
dat_param <- read.csv(here::here("Data Cleaning","NumParameters_final.csv"), stringsAsFactors = FALSE)
...
dat_flag <- read.csv(here::here("Data Cleaning","participant_revisedSubmissions.csv"), stringsAsFactors = FALSE)
...
for (i in 1:length(unique_teams_predicted_w1)) {
	dat_temp_1_w1 <- filter(dat_predicted_w1,
	...
	for (n in 1:length(domain_list_w1)) {
		...
		dat_accuracy_w1 <- dat_accuracy_w1 %>% add_row(
			...
			MASE1_w1 = computeMASE(
				dat_temp_2_w1$value,
				objective_data[1:40],
				objective_data[41:52],
				1
			)
		)
...
for (i in 1:length(domains)) {
	time_series <- ts(dat_hist[, domains[i]],
					  frequency = 12,
					  start = c(2017, 1))
	prediction_basis <- ts(dat_hist[1:40, domains[i]],
						   frequency = 12,
						   start = c(2017, 1))
...
rows_ideol <- which(dat$domain == "ideoldem" |
						dat$domain == "ideolrep")

# for each relevant row,
for (i in rows_ideol) {
	# Avoid generating accuracy scores for ground truth values
	if ((dat$Method.coded[i] != 5 |
		 is.na(dat$Method.coded[i])) & dat$phase[i] == 1) {
		domain <- dat$domain[i]
...
for (i in 1:length(domains)) {
	cutoff1 <- dat$MASE1_w1[which(dat$domain == domains[i] &
								  	dat$Method %in% "Naive - linear")]
	cutoff2 <- dat$MASE1_w1[which(dat$domain == domains[i] &
								  	dat$Method %in% "Naive - rwf")]
	
	
	list_val <- which(
		dat$phase == 1 &
			dat$domain == domains[i] &
			!dat$Method %in% c("Naive - rwf", "Naive - linear", "Objective")
	)
	
	
	for (n in 1:length(list_val)) {
		if (!is.na(dat$MASE1_w1[list_val[n]]) &
			dat$MASE1_w1[list_val[n]] < cutoff1) {
			dat$MASE_cutoff_Naive_linear[list_val[n]] <- 0
			
			
		} else if (!is.na(dat$MASE1_w1[list_val[n]]) &
				   dat$MASE1_w1[list_val[n]] > cutoff1) {
		...
		if (!is.na(dat$MASE1_w1[list_val[n]]) &
			dat$MASE1_w1[list_val[n]] < cutoff2) {
...
dat$compare_to_naive_linear_MASE <- factor(
	dat$MASE_cutoff_Naive_linear,
	levels = c(0, 1, 2),
	labels = c("Below Naive linear", "Above Naive linear", "Equal to Naive linear")
)

dat$compare_to_naive_rwf_MASE <- factor(
	dat$MASE_cutoff_Naive_rwf,
	levels = c(0, 1, 2),
	labels = c("Below Naive rwf", "Above Naive rwf", "Equal to Naive rwf")
)
...
for (i in 1:length(unique_teams_predicted_covid_death)) {
	dat_temp_1_covid <- filter(dat_covid,
	...
	for (n in 1:length(domain_list_covid)) {
		...
		dat_accuracy_covid <- dat_accuracy_covid %>% add_row(
			...
			MASE1_covid = computeMASE(
				dat_temp_2_covid$value,
				objective_data[37:40],
				objective_data[41:52],
				1
			)
		)
...
# create data set that will be merged with dat, that contains additional columns
#for mean error, root mean square error, mean percent error, mean absolute error,
#mean absolute percent error and the two types of mean absolute scaled error that
#are being calculated (#1 uses function declared on line 34, #2 uses
#Metrics::mase function)
...
for (i in 1:length(unique_teams_predicted_w1)) {
	dat_temp_1_w1_firstmonths <- filter(dat_predicted_w1_firstmonths,
										ResponseId == unique_teams_predicted_w1[i])
	
	domain_list_w1_firstmonths <- unique(dat_temp_1_w1_firstmonths$domain)
	
	for (n in 1:length(domain_list_w1_firstmonths)) {
		dat_temp_2_w1_firstmonths <- filter(dat_temp_1_w1_firstmonths,
											domain == domain_list_w1_firstmonths[n])
		...
		outcome_w1_firstmonths <- forecast::accuracy(dat_temp_2_w1_firstmonths$value,
													 domain_outcomes_w1_firstmonths[, which(names(domain_outcomes_w1_firstmonths) == domain_list_w1_firstmonths[n])])
		...
		dat_accuracy_w1_firstmonths <- dat_accuracy_w1_firstmonths %>% add_row(
			...
			MASE1_w1_firstmonths = computeMASE(
				dat_temp_2_w1_firstmonths$value,
				objective_data_firstmonths[1:40],
				objective_data_firstmonths[41:46],
				1
			)
		)
...
# merge dat_accuracy with dat by matching response ID, phase, and domain
dat <- dat %>% left_join(
	dat_accuracy_w1_firstmonths,
	by = c(
		"phase" = "phase",
		"ResponseId" = "ResponseId_w1_firstmonths",
		"domain" = "domain_w1_firstmonths"
	)
)
...
for (i in rows_ideol) {
	...
	if (dat$Method.coded[i] != 5 | is.na(dat$Method.coded[i])) {
		...
		# pull values for all months except month 9 (Jan 2021)
		val_w1_lastmonths <- c(dat$Month.7[i],
							   dat$Month.8[i],
							   dat$Month.10[i],
							   dat$Month.11[i],
							   dat$Month.12[i])
...
```{r add historic data to data frame bis}
...
```{r Naive forecast with linear extrapolation bis}
...
```{r Create naive forecasts with rwf bis}
...
for (i in 1:length(domains)) {
	time_series <- ts(dat_hist[, domains[i]],
					  frequency = 12,
					  start = c(2017, 1))
	prediction_basis <- ts(dat_hist[1:46, domains[i]],
						   frequency = 12,
						   start = c(2017, 1))
...
# Avoid generating accuracy scores for ground truth values
	if (dat$Method.coded[i] != 5 |
		is.na(dat$Method.coded[i])) {
		domain <- dat$domain[i]
		
		# pull values for all months except month 9 (Jan 2021)
		val_w2 <- c(dat$Month.7[i],
					dat$Month.8[i],
					dat$Month.10[i],
					dat$Month.11[i],
					dat$Month.12[i])
...
for (i in 1:length(domains)) {
	cutoff1_w2 <- dat$root_mean_sqr_error_w2[which(dat$domain == domains[i] &
												   	dat$Method %in% "Naive - linear" & dat$phase == 2)]
	cutoff2_w2 <- dat$root_mean_sqr_error_w2[which(dat$domain == domains[i] &
												   	dat$Method %in% "Naive - rwf" & dat$phase == 2)]
	...
	for (n in 1:length(list_val_w2)) {
		if (!is.na(dat$root_mean_sqr_error_w2[list_val_w2[n]]) &
			dat$root_mean_sqr_error_w2[list_val_w2[n]] < cutoff1_w2) {
			dat$RMSE_cutoff_Naive_linear_w2[list_val_w2[n]] <- 0
			
			
		} else if (!is.na(dat$root_mean_sqr_error_w2[list_val_w2[n]]) &
				   dat$root_mean_sqr_error_w2[list_val_w2[n]] > cutoff1_w2) {
		...
		if (!is.na(dat$root_mean_sqr_error_w2[list_val_w2[n]]) &
			dat$root_mean_sqr_error_w2[list_val_w2[n]] < cutoff2_w2) {
...
dat$compare_to_naive_linear_w2 <- factor(
	dat$RMSE_cutoff_Naive_linear_w2,
	levels = c(0, 1, 2),
	labels = c("Below Naive linear", "Above Naive linear", "Equal to Naive linear")
)

dat$compare_to_naive_rwf_w2 <- factor(
	dat$RMSE_cutoff_Naive_rwf_w2,
	levels = c(0, 1, 2),
	labels = c("Below Naive rwf", "Above Naive rwf", "Equal to Naive rwf")
)
...
for (i in 1:length(domains)) {
	cutoff1 <- dat$MASE1_w2[which(dat$domain == domains[i] &
								  	dat$Method %in% "Naive - linear" & dat$phase == 2)]
	cutoff2 <- dat$MASE1_w2[which(dat$domain == domains[i] &
								  	dat$Method %in% "Naive - rwf" & dat$phase == 2)]
	...
	for (n in 1:length(list_val)) {
		if (!is.na(dat$MASE1_w2[list_val[n]]) &
			dat$MASE1_w2[list_val[n]] < cutoff1) {
			dat$MASE_cutoff_Naive_linear_w2[list_val[n]] <- 0
			
			
		} else if (!is.na(dat$MASE1_w2[list_val[n]]) &
				   dat$MASE1_w2[list_val[n]] > cutoff1) {
		...
		if (!is.na(dat$MASE1_w2[list_val[n]]) &
			dat$MASE1_w2[list_val[n]] < cutoff2) {
...
dat$compare_to_naive_linear_MASE_w2 <- factor(
	dat$MASE_cutoff_Naive_linear_w2,
	levels = c(0, 1, 2),
	labels = c("Below Naive linear", "Above Naive linear", "Equal to Naive linear")
)

dat$compare_to_naive_rwf_MASE_w2 <- factor(
	dat$MASE_cutoff_Naive_rwf_w2,
	levels = c(0, 1, 2),
	labels = c("Below Naive rwf", "Above Naive rwf", "Equal to Naive rwf")
)
...
dat_final <- dat[, c(
	...
	"team_expertise",
	"team_size.coded",
	"team_discipline.coded",
	"team_discipline.missingdat",
	"previous_tournament.coded",
	"previous_tournament.missingdat",
	"Age",
	"Sex",
	"Genderident",
	"Genderident_5_TEXT",
	...
	"DataOnly",
	"mean_error_w1",
	"root_mean_sqr_error_w1",
	"mean_abs_error_w1",
	"mean_percent_error_w1",
	"mean_abs_percent_error_w1",
	"MASE1_w1",
	"RMSE_cutoff_Naive_linear",
	"RMSE_cutoff_Naive_rwf",
	"compare_to_naive_linear",
	"compare_to_naive_rwf",
	"MASE_cutoff_Naive_linear",
	"MASE_cutoff_Naive_rwf",
	"compare_to_naive_linear_MASE",
	"compare_to_naive_rwf_MASE",
	"time_spent",
	"mean_error_covid",
	"root_mean_sqr_error_covid",
	"mean_abs_error_covid",
	"mean_percent_error_covid",
	"mean_abs_percent_error_covid",
	"MASE1_covid",
	"mean_error_w1_lastmonths",
	"root_mean_sqr_error_w1_lastmonths",
	"mean_abs_error_w1_lastmonths",
	"mean_percent_error_w1_lastmonths",
	"mean_abs_percent_error_w1_lastmonths",
	"MASE1_w1_lastmonths",
	"mean_error_w2",
	"root_mean_sqr_error_w2",
	"mean_abs_error_w2",
	"mean_percent_error_w2",
	"mean_abs_percent_error_w2",
	"MASE1_w2",
	"RMSE_cutoff_Naive_linear_w2",
	"RMSE_cutoff_Naive_rwf_w2",
	"compare_to_naive_linear_w2",
	"compare_to_naive_rwf_w2",
	"MASE_cutoff_Naive_linear_w2",
	"MASE_cutoff_Naive_rwf_w2",
	"compare_to_naive_linear_MASE_w2",
	"compare_to_naive_rwf_MASE_w2",
	"team_gender",
	"team_education",
	"team_Age",
	"non_US",
	"CounterFactual_Presence_Final",
	"COVID.Final",
	"Onus.Final",
	"counterNum",
...
write.csv(dat_final, here::here("Wave1+2data.csv"))
write.csv(demographics_unique, here::here("Wave1+2demographics.csv"))
...
write.csv(dat_comp_1, here::here("team_size.csv"))
```
