...
DF_predicted_1 <- import(here::here("original_files", "Wave1+2data.csv")) %>%
	filter(phase == 1) %>% 
	select(team_name, domain, phase, Month.1:Month.12, basis_5_TEXT, model, theory)
...
DF_predicted_2 <- import(here::here("original_files", "Wave1+2data.csv")) %>%
	filter(phase == 2) %>% 
	select(team_name, domain, phase, Month.7:Month.18, basis_5_TEXT, model, theory) %>% 
...
DF_predicted %>% 
	...
	select(team_name, domain, phase, Predicted_Score)
```

The odds of teams entering the same scores, for all their estimates, for odd months and then separately for even months seemed very low. To investigate this problem on a larger scale, we calculated the correlation matrix of each team's scores. We then grabbed the lower triangle of the correlations (i.e., all unique pairwise correlations that do not include the month correlated with itself across domains). Any team with a perfect correlation between monthly estimates, we marked as inappropriate for the analyses we were trying to do for this robustness check. 
...
Therefore, `r round(nrow(correl_total)/length(correl_num)*100,2)`% of team and phase combinations are potentially unusable for this analysis.

The manuscript authors clarified that the information about the way teams created numbers was included in the dataset. For those teams, we output their reasons to a separate file for review (as it is too long to include in this report). 

```{r}
DF_predicted %>% 
	filter(team_name %in% correl_total$name) %>% 
	select(basis_5_TEXT, model, theory) %>% 
	unique() %>% 
	export(., "estimation_reasons.csv")
```

No variability in scores:

Another issue is that some teams' scores are simply the same score repeated for each month - as some people indicated in their answers - which would usually be excluded in a judgment of learning/memory task as not completing the task correctly.

For linear regression the slope is calculated as: $\hat{\beta}_i = \frac{\sum (x_i - \bar{x})(y_i - \bar{y})}{\sum (x_i - \bar{x})^2}$. If there is no variance in X (the estimated effect size scores), we cannot use the data because the denominator of the equation calculates to 0.
...
Therefore, `r round(nrow(sd_issues %>% filter(sd == 0))/nrow(sd_issues)*100,2)`% of team and phase combinations unusable for a linear regression analysis. Given these issues in the data, we did not proceed with the judgment-of-learning analysis, as the preceding comments made the results difficult to interpret.
